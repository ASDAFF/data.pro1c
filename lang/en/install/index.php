<?
/**
 * Copyright (c) 2/8/2019 Created By/Edited By ASDAFF asdaff.asad@yandex.ru
 */

$MESS ['IMPORT_PRO1C_MODULE_NAME'] = "Advanced exchange with 1C";
$MESS ['IMPORT_PRO1C_MODULE_DESCRIPTION'] = "Module can dramatically speed up the exchange of standard 1C, only update prices and residues monitor the exchange of real-time in Live log, have a log file for error analysis and performance, configure failover for exchange and see information about recent exchanges.";
$MESS ['IMPORT_PRO1C_PARTNER_NAME'] = "ASDAFF";
$MESS ['IMPORT_PRO1C_SITE_URI']= "https://asdaff.github.io/";
$MESS ['IMPORT_PRO1C_NEED_RIGHT_VER'] = "This solution needs main module version #NEED# or newer.";
$MESS ['IMPORT_PRO1C_NEED_MODULES'] = "This solution needs module #MODULE#.";

$MESS ['IMPORT_PRO1C_INSTALL_TITLE'] = "Installing the \"Debug exchange with 1C\"";
$MESS ['IMPORT_PRO1C_UNINSTALL_TITLE'] = "Removing the \"Debug exchange with 1C\"";

$MESS ['IMPORT_PRO1C_INSTALL_TABLE_ERROR'] = "Error when creating a table";
$MESS ['IMPORT_PRO1C_ONLY_MYSQL_ERROR'] = "This solution uses only MySQL database";
$MESS ['IMPORT_PRO1C_SAVE_COMPONENTS'] = "Save all of the components associated with the module";
$MESS ['IMPORT_PRO1C_SAVE_DATABASE_SETTINGS_OF_MODULE'] = "Save the settings from the database associated with the module";
$MESS ['IMPORT_PRO1C_BUTTON_DELETE_COMPONENTS'] = "Delete";

$MESS ['IMPORT_PRO1C_SETTINGS_PAGE'] = "Settings page";
?>